...
function comm_send(string $subject,string $body,array $recipients,bool $sendApp=true,bool $sendEmail=false):array{
    ...
    $appCount=0; $pushCount=0; $emailCount=0;
    foreach($recipients as $rec){
        $dip=(int)($rec['id']??0); if($dip<=0)continue; $label=(string)($rec['label']??''); $mail=trim((string)($rec['email']??''));
        $appOk=0; $pushOk=0; $mailOk=0; $link='communications.php?id='.$commId;
        if($sendApp){$appOk=app_notification_create($dip,$subject,$body,'info',$link)?1:0; $pushOk=send_browser_push_to_dipendente($dip,$subject,$body,$link)?1:0; $appCount+=$appOk; $pushCount+=$pushOk;}
        if($sendEmail&&$mail!==''){$mailOk=@mail($mail,$subject,$body,"Content-Type: text/plain; charset=UTF-8\r\n")?1:0; $emailCount+=$mailOk;}
