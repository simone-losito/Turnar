<?php
// modules/reports/report_operator_print.php
...
// HELPERS LOCALI
// --------------------------------------------------
function formatPausaPranzoOperatorPrint($value): string
{
    $pausa = (float)$value;
...
function reportOperatorPrintTipoConteggio(array $row): string
{
...
function reportOperatorPrintTipoClasse(array $row): string
{
    $countsAsWork = isset($row['counts_as_work']) ? (int)$row['counts_as_work'] : 0;
    $countsAsAbsence = isset($row['counts_as_absence']) ? (int)$row['counts_as_absence'] : 0;
    $isSpecial = isset($row['is_special']) ? (int)$row['is_special'] : 0;
...
$pauseUsate = [];
foreach ($rows as $row) {
    $pauseUsate[] = formatPausaPranzoOperatorPrint($row['pausa_pranzo'] ?? 0);
}
...
$pausaNote = 'Le ore nette di questo report sono calcolate usando la pausa pranzo impostata nel cantiere associato a ciascun turno dell’operatore. Inoltre il tipo di conteggio dipende dai flag della destinazione: counts_as_work = Lavoro, counts_as_absence = Assenza, destinazione speciale senza flag = Speciale neutra. La pausa viene scalata solo quando il turno supera la soglia minima necessaria per ottenere 8 ore effettive di lavoro più la pausa prevista.';
...
$totaliDestinazioni = [];
foreach ($rows as $row) {
    $nomeDest = trim((string)($row['destinazione'] ?? '-'));
    if ($nomeDest === '') {
        $nomeDest = '-';
    }

    if (!isset($totaliDestinazioni[$nomeDest])) {
        $totaliDestinazioni[$nomeDest] = [
            'ore' => 0.0,
            'tipo_conteggio' => reportOperatorPrintTipoConteggio($row),
            'classe' => reportOperatorPrintTipoClasse($row),
        ];
    }

    $totaliDestinazioni[$nomeDest]['ore'] += (float)($row['ore_nette'] ?? 0);
}
ksort($totaliDestinazioni);
?>
<!DOCTYPE html>
<html lang="it">
<head>
    <meta charset="UTF-8">
    <title>Report operatore stampabile</title>
    <style>
        :root{
            --line:#2c3446;
            --text:#141821;
        ...
        *{box-sizing:border-box}
        ...
        .tipo-lavoro{
            ...
            border-color:var(--work-bd);
        }

        .tipo-assenza{
            color:var(--absence);
            background:var(--absence-bg);
            border-color:var(--absence-bd);
        }

        .tipo-speciale{
            color:var(--special);
            background:var(--special-bg);
            border-color:var(--special-bd);
        }

        .tipo-operativa{
            color:var(--normal);
            background:var(--normal-bg);
            border-color:var(--normal-bd);
        }
...
<div class="toolbar">
    ...
    <a class="btn" href="<?php echo h(app_url('modules/reports/report_operator.php') . '?' . http_build_query($_GET)); ?>">Torna al report</a>
</div>
...
<div class="page">
    <div class="head">
        <div>
            ...
            <div class="subtitle">Report operatore stampabile</div>
        </div>
        <div class="period">
    ...
    <div class="section">
        <div class="section-title">Report ore operatore</div>
        <div class="section-subtitle">Riepilogo delle ore nette e dettaglio giornaliero del singolo operatore.</div>

        <div class="note">
            Le ore riportate in questo report sono calcolate secondo i <strong>flag della destinazione</strong> e già al netto della pausa pranzo del cantiere quando applicabile.
        </div>
        ...
        <div class="cards">
            ...
            <div class="card">
                ...
                <div class="card-value"><?php echo h(format_date_it($dataDa)); ?> → <?php echo h(format_date_it($dataA)); ?></div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-value"><?php echo (int)$totali['giorni']; ?></div>
            </div>
        ...
        <div class="block-title">Ore per destinazione</div>
        <table>
            <thead>
                <tr>
                    <th>Destinazione</th>
            ...
            <tbody>
                <?php if (!empty($totaliDestinazioni)): ?>
                    <?php foreach ($totaliDestinazioni as $nomeDest => $infoDest): ?>
                        <tr>
                            <td><?php echo h($nomeDest); ?></td>
                            <td>
                                <span class="tipo-pill <?php echo h((string)$infoDest['classe']); ?>">
                                    <?php echo h((string)$infoDest['tipo_conteggio']); ?>
                                </span>
                            </td>
                            <td class="num"><?php echo h(number_format((float)$infoDest['ore'], 2, ',', '.')); ?> h</td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
            ...
            <tbody>
                <?php if (!empty($rows)): ?>
                    <?php foreach ($rows as $row): ?>
                        <?php
                        $tipoConteggio = reportOperatorPrintTipoConteggio($row);
                        $tipoClasse = reportOperatorPrintTipoClasse($row);
                        ?>
                        <tr>
                            <td><?php echo h(format_date_it((string)$row['data'])); ?></td>
                            <td><?php echo h((string)($row['destinazione'] ?? '-')); ?></td>
                            <td>
                                <span class="tipo-pill <?php echo h($tipoClasse); ?>">
