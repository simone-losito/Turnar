...
<div class="entity-card operator-card">
    ...
    <div class="operator-info">
        <div class="operator-info-row">
            ...
            <div class="operator-info-value"><?= op_h($email ?: '-') ?></div>
        </div>

        <div class="operator-info-row">
            <div class="operator-info-label">Telefono</div>
            <div class="operator-info-value"><?= op_h($telefono ?: '-') ?></div>
        </div>

        <div class="operator-info-row">
            <div class="operator-info-label">Matricola</div>
            <div class="operator-info-value"><?= op_h($matricola ?: '-') ?></div>
        </div>

        <div class="operator-info-row">
            <div class="operator-info-label">Livello</div>
            <div class="operator-info-value"><?= op_h($livello ?: '-') ?></div>
        </div>

        <div class="operator-info-row">
            <div class="operator-info-label">Username</div>
            <div class="operator-info-value"><?= op_h($username ?: '-') ?></div>
        </div>
    </div>

    <div class="operator-footer">

        <span class="status-pill <?= $attivo ? 'is-active' : 'is-inactive' ?>">
        ...
        <div class="operator-actions">
            ...
            <?php if ($perm['edit']): ?>
                <a href="<?= app_url('modules/operators/edit.php?id='.$op['id']) ?>" class="btn btn-secondary btn-sm">Modifica</a>
            <?php endif; ?>

            <?php if (!empty($op['user_id']) && ($perm['viewUsers'] || $perm['editUsers'])): ?>
                <a href="<?= app_url('modules/users/edit.php?id='.$op['user_id']) ?>" class="btn btn-ghost btn-sm">Utente</a>
            <?php endif; ?>

            <?php if ($perm['delete']): ?>
                <a href="<?= app_url('modules/operators/delete.php?id='.$op['id']) ?>" 
                   class="btn btn-danger btn-sm"
                   onclick="return confirm('Eliminare questa persona?');">
                    Elimina
                </a>
            <?php endif; ?>
